...
def save_products_to_db(products: list[dict]) -> None:
    for product in products:
        try:
            ...
            existing_product = ScrapyWebFarmaciasas.objects.filter(nombre=product.name).first()
            if not existing_product:
                # Si no existe, crea un nuevo registro
                ScrapyWebFarmaciasas.objects.create(
                    nombre=product.name,
...
class Farmacia_sasGETSearchViewAll(APIView):
    ...
    def get(self, request, item: str):
        try:
            ...
            products: List[dict] = fetch_products_all_farmacia_sas(item)
